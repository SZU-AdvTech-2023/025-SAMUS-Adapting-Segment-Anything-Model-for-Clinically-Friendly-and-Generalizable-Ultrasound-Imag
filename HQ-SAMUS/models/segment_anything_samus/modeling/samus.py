...
class Samus(nn.Module):
    ...
    def forward(
        ...
        connectMasks = []
        imge, connectMask_1 = self.image_encoder(imgs)
        connectMasks.append(connectMask_1)
        if len(pt[0].shape) == 3:
          ...
          low_res_masks, _, connectMask_2 = self.mask_decoder( # low_res_mask b 1 128 128
          ...
          connectMasks.append(connectMask_2)
          outputs = {"low_res_logits": low_res_masks, "masks": masks, "connectMasks": connectMasks}
          return outputs
        else:
          low_res_masks, masks, connectMasks = [], [], []
          for i in range(pt[0].shape[1]):
            ...
            low_res_masksi, _, connectMasksi = self.mask_decoder( # low_res_mask b 1 128 128
                    image_embeddings=imge,
                    image_pe=self.prompt_encoder.get_dense_pe(), 
          ...
          connectMasks = connectMasks.reshape(connectMasks.shape[0], -1, connectMasks.shape[3], connectMasks.shape[4])
          outputs = {"low_res_logits": low_res_masks, "masks": masks, "connectMasks": connectMasks}
          return outputs
